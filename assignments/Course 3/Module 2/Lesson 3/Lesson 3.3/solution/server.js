...
// SOLUTION: broadcastCount + the connect/disconnect calls to it
// Broadcast the current user count to every open client.
function broadcastCount() {
    // Count only the clients whose connection is open.
    let count = 0;
...
wss.on('connection', (socket) => {
    ...
    // A client just connected, so broadcast the updated user count.
    broadcastCount();
    ...
    socket.on('close', () => {
        ...
        // A client just disconnected, so broadcast the updated user count.
        broadcastCount();
    });
});
